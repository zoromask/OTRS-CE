...
PREPARE FKStatement FROM @FKSQLStatement;
EXECUTE FKStatement;
SET @FKExists := (SELECT COUNT(*) FROM information_schema.table_constraints WHERE table_schema = DATABASE() AND table_name = 'oauth2_token' AND constraint_name = 'FK_oauth2_token_config_id_id');
SET @FKSQLStatement := IF( @FKExists = 0, 'ALTER TABLE `oauth2_token` ADD CONSTRAINT FK_oauth2_token_config_id_id FOREIGN KEY (config_id) REFERENCES `oauth2_token_config` (id)', 'SELECT ''INFO: Foreign key constraint FK_oauth2_token_config_id_id does already exist, skipping.''' );
PREPARE FKStatement FROM @FKSQLStatement;
EXECUTE FKStatement;
